    ...
    def find_maximums(self, model, num, exclusive):
        ...
        try:
            for _ in range(self.trial):
                ...
                if np.max(self.scores) > self.max:
                    self.max = np.max(self.scores)
                _score = (self.scores / self.max).tolist()
                genes = self.genes + self.elites
                if len(self.elite_scores) > 1:
                    self.elite_scores = (np.array(self.elite_scores) / self.max).tolist()
                ...
                for ind in elite_indexes:
                    ...
                    self.elite_scores.append(scores[ind] * self.max)
                # cross over
        ...
        # print(f"\nGA iter: {i}\tSearch space:{len(self.visited)}\telite_scores: {sorted(self.elite_scores)}\n")
        point = []
        ...
        return (
            ...
            (np.array(self.elite_scores)[idx] * (self.max)).tolist(),
        )
